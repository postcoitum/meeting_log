...
- **좌측**: 회의 목록(날짜별, 검색, 제목 수정, 삭제) · **중앙**: 로컬 AI 요약(마크다운 편집) + AI 채팅
- **우측 상단**: 내 메모 · **우측 하단**: 화자별 점유율·주요 발언 + 전사 스크립트 원본
- **마이크 녹음**: 상단바 녹음 버튼으로 회의를 직접 녹음 → 종료 시 자동 전사
- **AI 채팅**: 요약 아래 입력창에서 회의 내용에 대해 질문 (로컬 LLM 답변)
- **내보내기**: 요약·메모·스크립트를 마크다운 파일로 저장
- 상단바 토글로 사이드바 접기(요약은 유지), 투명도 슬라이더로 글래스 조절(설정 유지)
- 모든 데이터는 로컬 SQLite(`~/.meeting_log/meetings.db`)에 저장 (외부 전송 없음)
- 요약·채팅은 로컬 LLM(`mlx-community/Qwen2.5-3B-Instruct-4bit`)으로 생성
